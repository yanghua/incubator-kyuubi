	...
	private static InetAddress getGatewayAddress(DeploymentEntry deploy, Configuration flinkConfig) {
		...
		if (!address.isEmpty()) {
			try {
				...
				throw new RuntimeException("Invalid gateway address '" + address + "' for result retrieval.", e);
			}
		} else {
			// TODO cache this
			...
			if (jobManagerAddress != null && !jobManagerAddress.isEmpty()) {
				try {
					...
					throw new RuntimeException("Could not determine address of the gateway for result retrieval " +
